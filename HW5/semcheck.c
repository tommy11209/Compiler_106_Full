        ...
        if(simple && node->category != CONSTANT_t) {
            if(node->scope == 0) {
                if(isAssignmentLHS == __TRUE) {
                    fprintf(Outfp, "getstatic %s/%s ", program_name, node->name);
                    ...
                    case BOOLEAN_t:
                        ...
                        break;
                    case STRING_t:
                        //fprintf(Outfp, "\tslocal %d ; local variable number %d\n",node->name, node->symLocalNum);
                        break;
                    case REAL_t:
                        fprintf(Outfp, "F\n");
                        break;
                    default:
                        fprintf(Outfp, "[varifyExist LHS] fucking error\n");
                        break;
                    }
                } else {
                    fprintf(Outfp, "putstatic %s/%s ", program_name, node->name, node->name);
                    ...
                    case REAL_t:
                        ...
                        break;
                    default:
                        fprintf(Outfp, "[varifyExist RHS] fucking error\n");
                        break;
                    }
                }
            } else {
...
struct expr_sem *verifyFuncInvoke( const char *id, struct expr_sem *exprList, struct SymTable *table, int scope )
{
    ...
    fprintf(Outfp, "\tinvokestatic %s/%s(",program_name, node->name);
